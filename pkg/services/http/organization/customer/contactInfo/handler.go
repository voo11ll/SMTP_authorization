...
func CreateContactInfo(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		l.LogError(err.Error(), "pkg/Customer/Customerhttp/handler.CreateCustomerContactInfo")
	}
	defer conn.Close()

	_createReq := toGrpcCreateCustomerContactInfoRequest(&createReq)

	c := org.NewB24OrganizationServiceClient(conn)

	response, err := c.AddCustomerContactInfo(context.Background(), &org.AddCustomerContactInfoRequest{
...
func GetContactInfo(w http.ResponseWriter, r *http.Request) {
	...
	response, err := c.GetCustomerContactInfo(context.Background(), _getReq)

	getResp := customerHandler.ToGetContactInfoResponse(response)

	if response.Code == globalvars.Unauthorized {
		_getRes := &customerHandler.GetContactInfoResponse{
			Message:     globalvars.ErrTokenAccess.Error.Error(),
			Code:        globalvars.Unauthorized,
			ContactInfo: customerHandler.ContactInfoType{},
		}
		w.Header().Set("Content-Type", "application/json; charset=UTF-8")
...
func GetContactInfos(w http.ResponseWriter, r *http.Request) {
	...
	response, err := c.GetCustomerContactInfos(context.Background(), _getReq)

	getResp := customerHandler.ToGetContactInfosResponse(response)

	if response.Code == globalvars.Unauthorized {
		_getRes := &customerHandler.GetContactInfosResponse{
			Message:      globalvars.ErrTokenAccess.Error.Error(),
			Code:         globalvars.Unauthorized,
			ContactInfos: nil,
		}
...
func UpdateContactInfo(w http.ResponseWriter, r *http.Request) {
	...
	response, _ := c.UpdateCustomerContactInfo(context.Background(), &org.UpdateContactInfoRequest{
		Id:            updateReq.Id,
	...
	if response.Code == globalvars.ServerInternalError {
		_errRes := customerHandler.CustomerResponse{
			...
			Customer: customerHandler.CustomerType{},
		}
		w.Header().Set("Content-Type", "application/json; charset=UTF-8")

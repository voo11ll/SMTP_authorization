...
func CreateBank(w http.ResponseWriter, r *http.Request) {
	...
	_createReq := toGrpcCreateCustomerBankRequest(&createReq)

	c := org.NewB24OrganizationServiceClient(conn)

	response, err := c.AddCustomerBank(context.Background(), &org.AddCustomerBankRequest{
...
func GetBank(w http.ResponseWriter, r *http.Request) {
	...
	response, err := c.GetCustomerBank(context.Background(), _getReq)

	getResp := customerHandler.ToGetBankResponse(response)

	if response.Code == globalvars.Unauthorized {
		_getRes := &customerHandler.GetBankResponse{
			Message: globalvars.ErrTokenAccess.Error.Error(),
			Code:    globalvars.Unauthorized,
			Bank:    customerHandler.BankType{},
		}
		w.Header().Set("Content-Type", "application/json; charset=UTF-8")
...
func GetBanks(w http.ResponseWriter, r *http.Request) {
	...
	response, err := c.GetCustomerBanks(context.Background(), _getReq)

	getResp := customerHandler.ToGetBanksResponse(response)

	if response.Code == globalvars.Unauthorized {
		_getRes := &customerHandler.GetBanksResponse{
			Message: globalvars.ErrTokenAccess.Error.Error(),
			Code:    globalvars.Unauthorized,
			Banks:   nil,
		}
...
func UpdateBank(w http.ResponseWriter, r *http.Request) {
	...
	response, _ := c.UpdateCustomerBank(context.Background(), &org.UpdateBankRequest{
		Id:                updateReq.Id,
		Name:              updateReq.Name,
